...
enum OpModes { OPMODE_NONE, OPMODE_1650, OPMODE_1660, OPMODE_1670 };
...
private:
  ...
  ZResult setStatusRegister(const int snum, const int sval, int *crc8,
  ...
  void headerOut(const int channel, const int num, const int sz,
                 const int crc8);
  ...
  void checkPulseDial();
  uint8_t *doStateMachine(uint8_t *buf, uint16_t *bufLen, char **machineState,
                          String *machineQue, char *stateMachine);
  ...
  ZResult doTransmitCommand(int vval, uint8_t *vbuf, int vlen, bool isNumber,
                            const char *dmodifiers, int *crc8);
  ...
  ZResult doWiFiCommand(int vval, uint8_t *vbuf, int vlen, bool isNumber,
                        const char *dmodifiers);
  ZResult doDialStreamCommand(unsigned long vval, uint8_t *vbuf, int vlen,
                              bool isNumber, const char *dmodifiers);
  ZResult doPhonebookCommand(unsigned long vval, uint8_t *vbuf, int vlen,
                             bool isNumber, const char *dmodifiers);
  ZResult doAnswerCommand(int vval, uint8_t *vbuf, int vlen, bool isNumber,
                          const char *dmodifiers);
  ...
  ZResult doWebStream(int vval, uint8_t *vbuf, int vlen, bool isNumber,
                      const char *filename, bool cache);
  ZResult doTimeZoneSetupCommand(int vval, uint8_t *vbuf, int vlen,
                                 bool isNumber);
...
public:
  ...
  bool longResponses;
  bool doEcho;
  int speakerMode = 1; // 0=끔, 1=다이얼중만(기본), 2=항상켬
  bool isKorean = true; // 언어 설정 (true: 한글, false: 영어)
  String EOLN;
  char EC = '+';
  char ECS[32];
