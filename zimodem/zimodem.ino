...
// #define INCLUDE_CBMMODEM true  // ESP32 only, 1650, 1660, 1670, and Pulse
// Dialing support
...
// #define INCLUDE_OTH_UPDATES true // comment out if you make incompatible
// firmware
...
#elif defined(                                                                 \
    ARDUINO_ESP32C3_DEV) /* Configuration for the Esp32C3 4MB Dev Board */
#undef INCLUDE_SD_SHELL
...
#elif defined(                                                                 \
    ARDUINO_ESP32S3_DEV) /* Configuration for the Esp32S3 16MB Dev Board */
...
static String hostname = "AirLink-56K";
...
static unsigned long nextReconnectDelay =
...
static int getDefaultCtsPin() { return DEFAULT_PIN_CTS; }
...
static bool connectWifi(const char *ssid, const char *password, IPAddress *ip,
                        IPAddress *dns, IPAddress *gateWay, IPAddress *subNet) {
  ...
  bool amConnected =
      ...
      (strcmp(WiFi.localIP().toString().c_str(), "0.0.0.0") != 0);
  int WiFiCounter = 0;
  while ((!amConnected) && (WiFiCounter < 20)) {
    WiFiCounter++;
    if (!amConnected)
      delay(500);
    amConnected = (WiFi.status() == WL_CONNECTED) &&
                  (strcmp(WiFi.localIP().toString().c_str(), "0.0.0.0") != 0);
  }
  lastConnectAttempt = millis();
  if (lastConnectAttempt ==
      0) // it IS possible for millis() to be 0, but we need to ignore it.
    lastConnectAttempt = 1; // 0 is a special case, so skip it

  if (!amConnected) {
    nextReconnectDelay = 0; // assume no retry is desired.. let the caller set
                            // it up, as it could be bad PW
    WiFi.disconnect();
  } else
    nextReconnectDelay =
...
void setup() {
  ...
  pinSupport[0] = true;
  pinSupport[2] = true;
  if ((ESP.getFlashChipRealSize() / 1024) >=
      4096) // assume this is a strykelink/esp12e
  {
    pinSupport[4] = true;
    pinSupport[5] = true;
    for (int i = 9; i <= 16; i++)
...
#ifdef DEFAULT_PIN_RXD
  debugPrintf("Using BPS %d, RXD %d, TXD %d\r\n", DEFAULT_BAUD_RATE,
              DEFAULT_PIN_RXD, DEFAULT_PIN_TXD);
  HWSerial.begin(DEFAULT_BAUD_RATE, DEFAULT_SERIAL_CONFIG, DEFAULT_PIN_RXD,
                 DEFAULT_PIN_TXD);
#else
  ...
  snd_init(); // 사운드 엔진 초기화 (타이머 ISR 시작)
...
void checkReconnect() {
  if ((WiFi.status() != WL_CONNECTED) && (nextReconnectDelay > 0) &&
      (lastConnectAttempt > 0) && (wifiSSI.length() > 0)) {
