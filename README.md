...
* **기본 모뎀 통신 포트 (UART0)**
  ...
  * **흐름 제어 (Flow Control)**: `소프트웨어 방식 (XON/XOFF)`이 기본으로 활성화되어 있으며, 필요시 `AT&K` 명령어, 또는 `AT+CONFIG` 메뉴에서 하드웨어(RTS/CTS) 제어로 변경할 수 있습니다. **추천하는 것은 이후 아래에 설명된 `하드웨어 방식 (CTS/RTS)`을 사용하는 것입니다.**
...
* **시리얼 변환기**: MAX3232 (RS232 to TTL) **아래에 설명된 `하드웨어 방식 (CTS/RTS)`을 사용하기 위한 핀을 추가하는 작업을 추천드립니다.**
* **오디오**: 수동 부저 (Buzzer)
* **상태 표시등**: 5파이 LED 3개 (PWR, RX/TX, WIFI)
* **저항**: SMD 1206사이즈 560옴 저항 3개
* **나사**: M2 x 6 둥근머리 태핑나사 3개 

#### 📐 회로 설계 & 만능기판 배선
만능기판을 활용한 핀 배선 및 설계 레이아웃 예시입니다.<br/>
이 설계는 `소프트웨어 방식 (XON/XOFF)`기준이며, `하드웨어 방식 (CTS/RTS)`을 사용하실려면 배선 추가가 필요합니다.<br/>
노트북이나 컴퓨터 등에 바로 직결로 연결하실 예정이거나, 또는 작동이 되지않으면 MAX3232쪽 핀헤더에서 RXD와 TXD를 서로 바꿔서 연결하여야합니다.<br/>
![회로 설계](image/23.jpg)
...
#### 📐 하드웨어 방식 (CTS/RTS)핀을 추가하기 위한 MAX3232 수정
알리익스프레스나 타오바오, 국내등에서 저렴하게 구할 수 있는 4핀방식 MAX3232는 사실 CTS/RTS핀 추가가 가능합니다.<br/>
연결만 되어있지 않을 뿐이라, 사진같이 배선을 해주면 CTS/RTS핀을 뺄 수 있습니다.<br/>
![MAX3232 수정](image/34.jpg)<br/>
저의 경우는 사진과 같이 땜빵하여 직결로 연결하였습니다.<br/>
![MAX3232 수정](image/35.jpg)<br/>
![MAX3232 수정](image/36.jpg)
...
5. 연결 성공시 `와이파이 상태:`에 SSID가 표시됩니다. `엔터`를 눌러 저장하고, `y`를 눌러 저장합니다.<br/>
   ![최종 설정](image/15.jpg)
6. 만약 `하드웨어 방식 (CTS/RTS)`으로 작업하셨다면 위 설정에서 `흐름 제어`를 `하드웨어 (CTS/RTS)` 로 선택한 뒤 확인을 누릅니다.<br/>
이후 터미널 프로그램과 연결을 끊고, 위 2번에서 진행한 `모뎀 등록 정보` 내에서 `하드웨어 (CTS/RTS)`로 바꿉니다.<br/><br/>
